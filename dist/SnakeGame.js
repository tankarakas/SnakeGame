...
class SnakeGame {
    ...
    drawSnake(delta) {
        this.platform.snake.segments.forEach((segment, index) => {
            ...
            //   context.stroke();
            // } else {
            this.context.fillRect(segment.x * this.platform.blockSize, segment.y * this.platform.blockSize, this.platform.blockSize, this.platform.blockSize);
            // }
        });
    }
    drawApple(delta) {
        this.context.fillStyle = this.platform.apple.colour;
        this.context.fillRect(this.platform.apple.x * this.platform.blockSize, this.platform.apple.y * this.platform.blockSize, this.platform.blockSize, this.platform.blockSize);
    }
    drawCheckeredBackground(delta) {
        const colour1 = "gray";
        const colour2 = "blue";
        let currentColour = "gray";
        let colour = colour1;
        for (let currentXposition = 0; currentXposition <= this.platform.widthBlocks; currentXposition++) {
            for (let currentYposition = 0; currentYposition <= this.platform.heightBlocks; currentYposition++) {
                currentColour = currentColour === colour1 ? colour2 : colour1;
                this.context.fillStyle = colour;
                this.context.fillRect(this.platform.blockSize * currentXposition, this.platform.blockSize * currentYposition, this.platform.blockSize, this.platform.blockSize);
            }
        }
    }
    clearBoard(delta) {
